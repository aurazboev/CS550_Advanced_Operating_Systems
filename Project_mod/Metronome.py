...
class Metronome:
    def __init__(self):
        self.blockchain_server = "http://localhost:5002"
        self.pool_server = "http://localhost:5005"
        self.block_generation_interval = 30  # seconds
        self.metronome_thread = threading.Thread(target=self._metronome_loop, daemon=True)
        self.is_running = False
        self.validators = {}  # Dictionary to store registered validators
        self.validator_statistics = {}  # Dictionary to store validators' statistics
        self.start()
    ...
    def generate_and_process_block(self):
       ...
       if winning_validator:
          ...
          block_data = {"transactions": transactions, "validator_id": winning_validator, "fingerprint": validator_fingerprint}
       else:
