...
class Wallet:
    ...
    def create_wallet(self):
        ...
        print("Saved public key to config.yaml and private key to dsc-key.yaml in local folder")

    @app.route('/display_keys', methods=['GET'])
    ...
    def send_transaction(self, amount, recipient_address):
        ...
        transaction = {
            'sender': config['wallet']['public_key'],
        ...
        while transaction['status'] != 'confirmed':
            print(f"Transaction {transaction['tx_id']} status [{transaction['status']}]")
